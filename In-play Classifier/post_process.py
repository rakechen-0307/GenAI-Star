...
def list_to_ranges(lst):
    ...
    average_decibel_per_second, third_quartile = extract_decibels(
        config["original_video_path"]
    )
    ...
    for num in lst[1:]:
        if num == end + 1:
            ...
            if num == lst[-1]:
                if average_decibel_per_second[end - 1] > third_quartile:
                    ranges.append((start, end + 5))
                else:
            ...
            if ranges and (num - end) < merge_threshold:
                ...
                if average_decibel_per_second[end - 1] > third_quartile:
                    ranges.append((start, min(num, end + 5)))
                else:
                    ranges.append((start, min(num, end + 3)))
            start = num
            end = num
